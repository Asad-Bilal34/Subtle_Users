...
const AliForm = () => {
  ...
  const aliUID = 'u4JE8PL3BbSiRXllxHBMQ7ERZDJ3';

  useEffect(() => {
    const fetchData = async () => {
      const db = getDatabase();
      const userRef = ref(db, `users/${aliUID}/data`);
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md mt-4">
      <h2 className="text-xl font-semibold mb-4 text-center  text-blue-700">Ali's Submitted Data</h2>

      {data.length === 0 ? (
        <p className="text-center text-gray-500">No entries found for Ali.</p>
      ) : (
